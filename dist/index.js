...
export default function createServer() {
    ...
    async function getDOIsFromPubMed(pmids) {
                ...
                if (searchResponse.ok) {
                    ...
                    if (pmids.length > 0) {
                        ...
                        if (fetchResponse.ok) {
                            ...
                            return { source: 'pubmed', data: pmids.map((id) => fetchData.result?.[id]).filter(Boolean) };
                        }
                    }
                }
    ...
    async function normalizeResult(result, source, doiMap) {
        if (!result)
            return null;
        switch (source) {
            case 'crossref':
            ...
            case 'zbmath':
                return {
                    source: 'zbMATH',
                    title: result.title,
                    authors: result.authors?.map((a) => a.name || a).filter(Boolean),
            ...
            case 'inspirehep':
                ...
                return {
                    ...
                    authors: metadata.authors?.map((a) => a.full_name).filter(Boolean),
            ...
            case 'semanticscholar':
                return {
                    ...
                    title: result.title,
                    authors: result.authors?.map((a) => a.name).filter(Boolean),
                    year: result.year,
                ...
                return {
                    ...
                    authors: result.info?.authors?.author ?
                        (Array.isArray(result.info.authors.author) ?
                            result.info.authors.author.map((a) => a.text || a) :
    ...
    function calculateMatchScore(result, filters) {
        ...
        if (filters.authors && filters.authors.length > 0 && result.authors && result.authors.length > 0) {
            ...
            const matchedAuthors = filters.authors.filter((filterAuthor) => {
                ...
                return resultAuthorsLower.some((resultAuthor) => resultAuthor.includes(lastName));
            });
    ...
    server.registerTool("verifyCitation", {
        ...
        try {
            ...
            const pubmedPmids = searchResults.pubmed?.map((r) => r.uid).filter(Boolean) || [];
            ...
            if (searchResults.crossref) {
                normalizationPromises.push(...searchResults.crossref.map((r) => normalizeResult(r, 'crossref')));
            }
            if (searchResults.openalex) {
                normalizationPromises.push(...searchResults.openalex.map((r) => normalizeResult(r, 'openalex')));
            }
            if (searchResults.pubmed) {
                normalizationPromises.push(...searchResults.pubmed.map((r) => normalizeResult(r, 'pubmed', pubmedDoiMap)));
            }
            if (searchResults.zbmath) {
                normalizationPromises.push(...searchResults.zbmath.map((r) => normalizeResult(r, 'zbmath')));
            }
            if (searchResults.eric) {
                normalizationPromises.push(...searchResults.eric.map((r) => normalizeResult(r, 'eric')));
            }
            if (searchResults.hal) {
                normalizationPromises.push(...searchResults.hal.map((r) => normalizeResult(r, 'hal')));
            }
            if (searchResults.inspirehep) {
                normalizationPromises.push(...searchResults.inspirehep.map((r) => normalizeResult(r, 'inspirehep')));
            }
            if (searchResults.semanticscholar) {
                normalizationPromises.push(...searchResults.semanticscholar.map((r) => normalizeResult(r, 'semanticscholar')));
            }
            if (searchResults.dblp) {
                normalizationPromises.push(...searchResults.dblp.map((r) => normalizeResult(r, 'dblp')));
            }
            const allResults = (await Promise.all(normalizationPromises)).filter((r) => r !== null);
    ...
    server.registerTool("batchVerifyCitations", {
        ...
        try {
            const results = await Promise.all(citations.map(async (citation, index) => {
    ...
    server.registerTool("findVerifiedPapers", {
        ...
        try {
                ...
                apiPromises.push((async () => {
                    ...
                    if (searchResponse.ok) {
                        ...
                        if (pmids.length > 0) {
                            ...
                            if (fetchResponse.ok) {
                                const fetchData = await fetchResponse.json();
                                return { source: 'pubmed', items: pmids.map((id) => fetchData.result?.[id]).filter(Boolean) };
                            }
                        }
                    }
            ...
            if (source === 'all' || source === 'zbmath') {
                apiPromises.push((async () => {
                    ...
                    if (yearFrom)
                        zbmathUrl += `&year=${yearFrom}`;
                    if (yearTo && !yearFrom)
                        zbmathUrl += `&year=${yearTo}`;
                    const response = await fetch(zbmathUrl);
                    if (response.ok) {
                        const data = await response.json();
            ...
            const pubmedPmids = pubmedItems?.items.map((r) => r.uid).filter(Boolean) || [];
            const pubmedDoiMap = await getDOIsFromPubMed(pubmedPmids);
            // Normalize all results in parallel
            const normalizationPromises = [];
            allItems.forEach(({ source, items }) => {
                if (source === 'pubmed') {
                    normalizationPromises.push(...items.map((item) => normalizeResult(item, 'pubmed', pubmedDoiMap)));
                }
                else {
                    normalizationPromises.push(...items.map((item) => normalizeResult(item, source)));
                }
            });
            const allPapers = (await Promise.all(normalizationPromises)).filter((r) => r !== null);
    ...
    // Register Resources
    server.registerResource("Supported Citation Databases", "citation://databases", {
    ...
    server.registerResource("Citation Verification Guidelines", "citation://guidelines", {
...
`
    ...
    server.registerPrompt("citation-verification-rules", {
